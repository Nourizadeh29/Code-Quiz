...
const questions = [
  {
    questionText: "Inside which HTML elements do we put the Javascript ?",
...
function displayQuestion() {
  ...
  h2QuestionElement.textContent = `Q.${currentQuestion + 1} `+ question.questionText;
...
function renderLeaderboard() {
  let ScoreArray = getLeaderboard();
  const scoreList = document.querySelector("#highscore-list");
  scoreList.innerHTML = "";
  for (let i = 0; i < ScoreArray.length; i++) {
    let leaderboardEntry = ScoreArray[i];
